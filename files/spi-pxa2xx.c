...
static void cs_assert(struct spi_device *spi)
{
	...
	if (drv_data->ssp_type == CE4100_SSP) {
		pxa2xx_spi_write(drv_data, SSSR, (u32)spi->chip_select);
		return;
	}
...
static irqreturn_t ssp_int(int irq, void *dev_id)
{
	...
	if (pm_runtime_suspended(drv_data->dev))
#else
...
static void u8value(u32 val, void* dest)
{
	*(u8 *)dest = val;
}
 
static void u16value(u32 val, void* dest)
{
	*(u16 *)dest = val;
}
 
static void u32value(u32 val, void* dest)
{
	*(u32 *)dest = val;
}

static void (*ReadVal)(u32, void*);
static int up_spi_transfer(struct driver_data *drv_data, 
			   struct chip_data *chip, 
			   struct spi_transfer *transfer)
{
	...
	while(len--)
	{
	    ...
	    {
		if((pxa_ssp_read_reg(drv_data->ssp,SSSR) & SSSR_RNE))
		{
		    //rx
...
pxa2xx_spi_init_pdata(struct platform_device *pdev)
{
	...
	const void *match;
	int status;
	bool is_lpss_priv = platform_get_resource_byname(pdev, IORESOURCE_MEM, "lpss_priv");
	...
	if (match)
		...
		status = device_property_read_u32(&pdev->dev, "intel,spi-pxa2xx-type", &value);
		if (status)
...
static int pxa2xx_spi_probe(struct platform_device *pdev)
{
	...
	if (platform_info->is_slave)
		controller = spi_alloc_slave(dev, sizeof(*drv_data));
	else
